...
const SideBox = (data) => {
  return (
    <div className="sidebar">
      ...
      >
        <Avatar sx={{ width: 116, height: 116 }}>
      ...
      <Box sx={{ flexGrow: 1 }}>
        <Grid container>
          ...
          <Grid
            item
            xs={6}
            style={{ borderRight: "0.1px solid grey", minHeight: "40vh" }}
          >
                    ...
                    <ListItemText
                      primary={
                        <Typography
                          type="body2"
                          style={{ color: "black" }}
                        >{`Max Allowed Credit : ₹ ${credit}`}</Typography>
                      }
                    />
                  </ListItemButton>
                </ListItem>
...
const MainBoxSecond = ({ 
  account = {
    ...
    "Account Type": "Saving",
    "Account Email": "manmohan@gmail.com"
  }, 
  indicators 
}) => {
            ...
            <Box>
              ...
              <List>
                {indicators.map((item, index) => {
                  ...
                  return (
                    <Box p={1}>
                      <Typography ml={2} mb={1} style={{ fontWeight: "bold" }}>
                        {" "}
                        {`${item["name"]} (${r.toString()})`}
                      </Typography>
                      <Progressbar percentage={item["rating"]} />
                    </Box>
                  );
                })}
              </List>
...
const FullReport = ({ reportData, reportId }) => {
  ...
  const handleAcceptSubmit = () => {
    ...
    const apiUrl = encodeURI(
      `http://localhost:8000/updateuser?userid=${reportId}&updateType=accept&approvedAmount=${v}`
    );
    console.log(apiUrl);
  ...
  const handleRejectSubmit = () => {
    ...
    const apiUrl = encodeURI(
      `http://localhost:8000/updateuser?userid=${reportId}&updateType=reject&approvedAmount=0`
    );
    console.log(apiUrl);
    axios
      .get(apiUrl)
  ...
  const handleHandleSubmit = () => {
    ...
    setHaltText("Halting...");
    const apiUrl = encodeURI(
      `http://localhost:8000/updateuser?userid=${reportId}&updateType=halt&approvedAmount=${v}`
        ...
        <Box sx={{ flexGrow: 1 }}>
          <Grid container spacing={2}>
            <Grid item xs={3}>
              ...
              >
                <SideBox data={reportData["userFormSubmittedInfo"]} />
              </Item>
            </Grid>
            <Grid item xs={9}>
              <Item
              ...
              >
                <MainBox
                  piedata={reportData["transactionalAnalysis"]}
            ...
            >
              ...
              <Grid item container>
                {
                  getIndicatorList(reportData["indicators"]).map(
                    (item, index) => {
                      return (
                        <SectorCard
                          name={item.name}
      ...
      >
        ...
        >
          ...
          <Button
            variant="contained"
            color="success"
            onClick={() => handleOpen(reportData["allowedCredit"])}
          >
            Accept
          </Button>
        </Grid>
      ...
      >
        <Box sx={style}>
          ...
          <TextField
            ...
            value={allowedCredit}
            onChange={(event) => {
              setAllowedCredit(event.target.value);
            }}
          />
          <br />
          <br />
